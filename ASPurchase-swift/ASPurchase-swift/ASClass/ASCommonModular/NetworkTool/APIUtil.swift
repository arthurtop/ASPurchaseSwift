...
extension APIUtil {
    ...
    static func fetchData<T: HandyJSON>(with url: Router, method: HTTPMethod = .get, parameters: Parameters? = nil, encoding: ParameterEncoding = URLEncoding.queryString, returnType: T.Type) -> Observable<T> {
        return Observable<T>.create({ (observer: AnyObserver<T>) -> Disposable in
            self.request(observer: observer, url: url, method: method, parameters: parameters, encoding: encoding, returnType: returnType)
...
extension APIUtil {
    ...
    ///   - result: 请求结果
    ///   - retrunType: 返回值类型
    fileprivate static func successHandle<T: HandyJSON>(observer: AnyObserver<T>, result: AFDataResponse<Any>, retrunType: T.Type) {
        // 如果解析出来的不是json
...
extension APIUtil {
    ...
    fileprivate static func request<T: HandyJSON>(observer: AnyObserver<T>, url: Router, method: HTTPMethod, parameters: Parameters?, encoding: ParameterEncoding, returnType: T.Type) {
        AF.request(url, method: method, parameters: parameters, encoding: encoding, headers: self.httpsHeaders).responseJSON { (response) in
            switch response.result {
            case .success:
                self.successHandle(observer: observer, result: response, retrunType: returnType)
                break
